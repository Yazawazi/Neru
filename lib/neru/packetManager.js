...
const PacketManager = class {
    constructor (config) {
        ...
        this.structPath = config.structPath || "D:/sora/struct.json"

        this.dataManager = new DataManager({
            errorHandle: this.errorHandle,
            structPath: this.structPath
        })
    ...
    parse(data) {
        ...
        data.forEach(packet => {
            ...
            if ("rpcData" in result.body) result.fields =
                this.dataManager.parse(result.body.rpcData, protocolId, result.isRpcRet)
            packets.push(result)
        })
        return packets
    }
